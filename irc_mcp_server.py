...
def _get_or_create_session_key() -> "_Fernet | None":
    ...
    The key is generated by install_mcp.sh (running as root) — the service
    process only reads it, never writes to /etc or /var/lib at startup.
    Returns None if the package is unavailable or key file is missing.
    """
    ...
    if not key_path.exists():
        log.error(
            f"Session encryption key not found: {SESSION_KEY_FILE}. "
            "Run install_mcp.sh to generate it. "
            "Sessions will NOT be encrypted until the key exists."
        )
        return None
    try:
        return _Fernet(key_path.read_bytes())
    except Exception as e:
        log.error(f"Failed to load session key from {SESSION_KEY_FILE}: {e}")
        return None
...
def _save_sessions(data: dict) -> None:
    """
    Save sessions to disk, encrypting sensitive fields.
    The on-disk file never contains plaintext passwords or emails.
    """
    try:
        ...
        Path(SESSIONS_PATH).write_text(json.dumps(encrypted, indent=2))
    except Exception as e:
