...
_SENSITIVE_FIELDS = {"ns_password", "ns_email"}


def _get_or_create_session_key() -> "_Fernet | None":
    """
    Load the Fernet session encryption key from SESSION_KEY_FILE.
    Key is generated by install_mcp.sh — service only reads, never writes.
    Returns None only if cryptography package unavailable (checked at startup).
    Corrupt key file = hard failure logged here, caught by _check_runtime_security.
    """
    if not _FERNET_AVAILABLE:
        return None
    key_path = Path(SESSION_KEY_FILE)
    if not key_path.exists():
        return None   # _check_runtime_security will catch this and exit
    try:
        return _Fernet(key_path.read_bytes())
    except Exception as e:
        log.error(f"FATAL: Session key at {SESSION_KEY_FILE} is corrupt: {e}")
        log.error("Restore the key from backup or re-run install_mcp.sh.")
        sys.exit(1)


# Initialise the Fernet instance at module load time
_fernet: "_Fernet | None" = _get_or_create_session_key()


def _encrypt_field(value: str) -> str:
    """Encrypt a string value. Returns 'enc:<token>' or plain value if unavailable."""
...
def _decrypt_field(value: str) -> str:
    """Decrypt an 'enc:<token>' value. Plain values pass through unchanged."""
    if not value or not value.startswith("enc:"):
        return value      # plaintext — either not encrypted or empty
    if not _fernet:
        log.error("Cannot decrypt session field — cryptography package unavailable.")
        return ""
    try:
        return _fernet.decrypt(value[4:].encode()).decode()
    except Exception as e:
        log.error(f"Failed to decrypt session field: {e}")
        return ""


def _encrypt_meta(meta: dict) -> dict:
    """Return a copy of a session metadata dict with sensitive fields encrypted."""
    out = dict(meta)
...
def _decrypt_meta(meta: dict) -> dict:
    """Return a copy of a session metadata dict with sensitive fields decrypted."""
    out = dict(meta)
    for field in _SENSITIVE_FIELDS:
        if field in out:
...
def _load_sessions() -> dict:
    """
    Load sessions from disk, decrypting sensitive fields.
    Handles plaintext legacy sessions gracefully (migration path).
    """
    try:
        p = Path(SESSIONS_PATH)
        if p.exists():
            raw = json.loads(p.read_text())
            # Decrypt each session's sensitive fields
            return {sid: _decrypt_meta(meta) for sid, meta in raw.items()}
    except Exception as e:
...
def _save_sessions(data: dict) -> None:
    """
    Save sessions to disk with sensitive fields encrypted.
    Hard failure if encryption is unavailable — never writes plaintext.
    """
    if not _fernet:
        log.error(
            "FATAL: Cannot save sessions — encryption not available. "
            "Check that session.key exists and cryptography is installed."
        )
        return   # Don't write plaintext — just skip the save
    try:
...
def _check_runtime_security() -> None:
    """
    Check security conditions at startup.
    ...
    if not Path(SESSION_KEY_FILE).exists():
        log.error("=" * 60)
        log.error("FATAL: Session encryption key not found.")
        log.error(f"Expected: {SESSION_KEY_FILE}")
        log.error("The installer generates this key. If you are seeing this:")
        log.error("  1. Re-run install_mcp.sh to generate the key, OR")
        log.error("  2. Restore the key from your backup")
        log.error("Running without encryption is not permitted.")
        log.error("=" * 60)
        sys.exit(1)

    if not _FERNET_AVAILABLE:
        log.error("=" * 60)
        log.error("FATAL: 'cryptography' package is not installed.")
        log.error("Session data cannot be encrypted without it.")
        log.error("Run: /opt/claude-irc-mcp/venv/bin/pip install cryptography")
        log.error("=" * 60)
        sys.exit(1)

    # ── Warnings: bad permissions on sensitive files ───────────────────────
    _warn_permissions(SESSION_KEY_FILE,  {"0o640", "0o600"}, "session key")
    _warn_permissions(CONFIG_PATH,       {"0o640", "0o600"}, "config file")
    _warn_permissions(SESSIONS_PATH,     {"0o640", "0o600"}, "sessions file", must_exist=False)

    # ── Warning: running as root ───────────────────────────────────────────
    if os.geteuid() == 0:
        log.warning(
            "WARNING: MCP server is running as root. "
            "This is a security risk — run as an unprivileged user (claudemcp)."
        )


def _warn_permissions(path: str, good_modes: set, label: str, must_exist: bool = True) -> None:
    """Warn if a sensitive file has overly permissive modes."""
    p = Path(path)
    if not p.exists():
        if must_exist:
            log.warning(f"WARNING: {label} not found at {path}")
        return
    try:
        mode = oct(os.stat(path).st_mode & 0o777)
        if mode not in good_modes:
            log.warning(
                f"WARNING: {label} {path} has permissions {mode} — "
                f"recommended: {'/'.join(sorted(good_modes))}. "
                f"Fix with: chmod 640 {path}"
            )
    except Exception as e:
        log.warning(f"Could not check permissions on {label} {path}: {e}")
